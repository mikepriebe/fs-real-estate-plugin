...
$ModSQL = '';
if (function_exists('fsrep_listing_hidden_mod')) { $ModSQL .= fsrep_listing_hidden_mod(); }
if (function_exists('fsrep_member_moderation_check')) { $ModSQL .= fsrep_member_moderation_check(); }
if (function_exists('fsrep_listing_visibility_mod')) { $ModSQL .= fsrep_listing_visibility_mod(); }
if (isset($_GET['id'])) {
	...
	if (is_numeric($_GET['filter'])) {
		...
		if ($_GET['filter'] != 0) {
			...
			$query = "SELECT * FROM ".$table_prefix."fsrep_listings WHERE listing_long != '' AND listing_lat != '' AND listing_id IN ($MatchingListingsID)".$ModSQL;
		}
	}
} elseif (isset($_GET['cityid'])) {
	if (is_numeric($_GET['cityid'])) {
		$query = "SELECT * FROM ".$table_prefix."fsrep_listings WHERE listing_long != '' AND listing_lat != '' AND listing_address_city = ".$_GET['cityid'].$ModSQL;
	}
} elseif (isset($_GET['provinceid'])) {
	if (is_numeric($_GET['provinceid'])) {
		$query = "SELECT * FROM ".$table_prefix."fsrep_listings WHERE listing_long != '' AND listing_lat != '' AND listing_address_province = ".$_GET['provinceid'].$ModSQL;
	}
} elseif (isset($_GET['countryid'])) {
	if (is_numeric($_GET['countryid'])) {
		$query = "SELECT * FROM ".$table_prefix."fsrep_listings WHERE listing_long != '' AND listing_lat != '' AND listing_address_country = ".$_GET['countryid'].$ModSQL;
	}
} else {
	if (isset($_SESSION['FSREPSearch']) && $_SESSION['FSREPSearch'] != '') { 
		$query = $_SESSION['FSREPSearch']; 
	} else {
		$query = "SELECT * FROM ".$table_prefix."fsrep_listings WHERE listing_long != '' AND listing_lat != ''".$ModSQL;
	}
}
