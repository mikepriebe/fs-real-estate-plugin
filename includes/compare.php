...
if ($count == 0) {
	echo '<p>No listings were selected to compare.</p>';
} else {
	...
	*/
	...
	echo '<tr>'; for($i=1;$i<=$count;$i++) { $ListingPrice = ''; if ($ListingDetails[$i]->listing_price_num != '0.00') { $ListingPrice = $FSREPconfig['Currency'].fsrep_currency_format($ListingDetails[$i]->listing_price_num).$CurrencyDisplay; } echo '<td width="'.$TDWidth.'">'.$ListingPrice.'</td>'; } echo '</tr>';
