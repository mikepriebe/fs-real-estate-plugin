...
if ($count == 0) {
	...
	if (file_exists($WPUploadDir['basedir'].'/fsrep/houses/small/'.$ListingDetails[$i]->listing_id.'.jpg')) {
	...
	for($i=1;$i<=$count;$i++) {
		$AdditionalImages = '';
		for ($i=1;$i<=10;$i++) {
			if (file_exists($WPUploadDir['basedir'].'/fsrep/houses/additional/small/'.$ListingDetails[$i]->listing_id.'-'.$i.'.jpg')) {
