	...
	echo '<table class="widefat page fixed" cellspacing="0" border="1">
		...
		foreach($Fields as $Fields) {
			...
			if (isset($_GET['cform'])) { echo ''; } else { 
				...
				if (function_exists('fsrep_pro_field_listings')) { echo fsrep_pro_field_listings($Fields);} else { echo '<input type="checkbox" disabled> <span style="color: #999999">Viewable (PRO Only)</span>'; }
			}
			echo '</td>';
			// ORDERING BUTTONS
