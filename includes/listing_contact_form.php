...
if (isset($_POST['submit'])) {
	...
	if ($FSREPSendMail == TRUE) {
		function fsrep_set_content_type() {
			return 'text/html';
		}
		$subject = "Listing #".$ListingDetails->listing_id." Inquiry (".date("m/d/y").")"; 
		$headers = 'From: '.get_bloginfo('name').' <no-reply@'.$ReplyEmail[1].'>' . "\r\n";
		add_filter('wp_mail_content_type', 'fsrep_set_content_type');
		wp_mail($EmailOrderRecipients, $subject, $CFormMessage, $headers);
		if ($FSREPconfig['CopyAdminOnListingMessages'] == 'Yes') {
			wp_mail(get_bloginfo('admin_email'), $subject, $CFormMessage, $headers);
		}
		remove_filter('wp_mail_content_type', 'fsrep_set_content_type');
	}
}
