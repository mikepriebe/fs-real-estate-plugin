<?php
	...
	if ($ListingDetails->listing_address_number == '' || $ListingDetails->listing_address_street == '' || $ListingDetails->listing_address_city == '' || $ListingDetails->listing_address_province == '' || $ListingDetails->listing_address_postal == '') { $FSREPShowMap = FALSE; }
	...
	$PageContent .= '<span class="listingprice">'; if ($FSREPconfig['ListingPriceID'] != '') { $PageContent .= $FSREPconfig['ListingPriceID'].' '; } $PageContent .= $FSREPconfig['Currency'].fsrep_currency_format($ListingDetails->listing_price_num).$CurrencyDisplay.'</span><br />';
	...
	for ($i=1;$i<=50;$i++) {
		if (file_exists(ABSPATH.'wp-content/uploads/fsrep/houses/additional/small/'.$ListingDetails->listing_id.'-'.$i.'.jpg')) {
			$PageContent .= '<div class="fsrep-aimage"><a href="'.get_option('home').'/wp-content/uploads/fsrep/houses/additional/large/'.$ListingDetails->listing_id.'-'.$i.'.jpg" title="View Slideshow" class="thickbox" rel="fsreplisting" onmouseover="document.getElementById(\'fsrep-main-image-img\').src=\''.get_option('home').'/wp-content/uploads/fsrep/houses/additional/'.$ListingDetails->listing_id.'-'.$i.'.jpg\'; document.getElementById(\'fsrep-main-image-a\').rel=\'lightbox[slideshow]\'"><img src="'.get_option('home').'/wp-content/uploads/fsrep/houses/additional/small/'.$ListingDetails->listing_id.'-'.$i.'.jpg" class="full" /></a></div>';
		}
	}
	if (file_exists(ABSPATH.'/wp-content/uploads/fsrep/houses/large/'.$ListingDetails->listing_id.'.jpg')) { 
		$PageContent .= '<div class="fsrep-aimage"><a href="'.get_option('home').'/wp-content/uploads/fsrep/houses/large/'.$ListingDetails->listing_id.'.jpg" title="View Slideshow" class="thickbox" rel="fsreplisting" onmouseover="document.getElementById(\'fsrep-main-image-img\').src=\''.get_option('home').'/wp-content/uploads/fsrep/houses/'.$ListingDetails->listing_id.'.jpg\'; document.getElementById(\'fsrep-main-image-a\').rel=\'lightbox[slideshow]\'"><img src="'.get_option('home').'/wp-content/uploads/fsrep/houses/small/'.$ListingDetails->listing_id.'.jpg" class="full" /></a></div>';
	}
	$PageContent .= '</div>';
	$PageContent .= '</div>';
