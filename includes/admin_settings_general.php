<?php
	...
	if (isset($_POST['submit'])) {
		...
		if (isset($_POST['FooterLink'])) { $wpdb->query("UPDATE ".$wpdb->prefix."fsrep_config SET config_value = '".addslashes($_POST['FooterLink'])."' WHERE config_name = 'FooterLink'"); }
	...
	if ($FSREPPermalinkStructure == '') {
		...
		$PermaSolution = '<a href="admin.php?page=fsrep_settings&permalinks=fix" class="button-primary">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Automatically Fix Label', 'Automatically Fix').'</a>';
	} else {
	...
	echo '<tr><td>'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Repair Database Label', 'Repair Database').'</td><td><a href="admin.php?page=fsrep_settings&repairdb" class="button-primary">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Repair Label', 'Repair').'</a></td><td>&nbsp;</td>';
	...
	echo '<table class="widefat page fixed" cellspacing="0" border="1">
		...
		<tbody>';
	$FSREPThemeChoices = array('Default' => 'default');
	if ($FSREPThemes = opendir(ABSPATH.'/wp-content/plugins/fs-real-estate-plugin/themes/')) {
		while (false !== ($FSREPThemeDir = readdir($FSREPThemes))) {
			if ($FSREPThemeDir != '.' && $FSREPThemeDir != '..' && $FSREPThemeDir != 'default') {
				if (file_exists(ABSPATH.'/wp-content/plugins/fs-real-estate-plugin/themes/'.$FSREPThemeDir.'/style.css')) {
					$FSREPThemeStyle = file(ABSPATH.'/wp-content/plugins/fs-real-estate-plugin/themes/'.$FSREPThemeDir.'/style.css');
					$FSREPAddTheme = array(str_replace('Template Name: ','',$FSREPThemeStyle[1]) => $FSREPThemeDir);
					$FSREPThemeChoices = array_merge($FSREPThemeChoices, $FSREPAddTheme);
				}
			}
		}
		closedir($FSREPThemes);
	}
