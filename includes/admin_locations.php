...
function fsrep_local() {
	...
	<th scope="col" class="manage-column"><input type="submit" name="submit" class="button-primary" value="Update Locations" style="padding: 3px 8px;"></th>
	</tr>
	</thead>
	<tfoot>
	<tr>
	...
	foreach ($Countries as $Countries) {
		...
		if (count($Provinces) > 0) {
			foreach ($Provinces as $Provinces) {
				...
				echo '<td style="background: #F5F5F5;">&nbsp;</td></tr>';
				$Cities = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."fsrep_cities WHERE province_id = $Provinces->province_id ORDER BY city_name");
				if (count($Cities) > 0) {
					foreach ($Cities as $Cities) {
