...
function fsrep_local() {
	...
	$Countries = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."fsrep_countries ORDER BY country_name");
	foreach ($Countries as $Countries) {
		echo '<tr><td style="background: #EFEFEF;"><a href="admin.php?page=fsrep_local&f=del&type=c&lid='.$Countries->country_id.'" onClick="return confirm(\'Are you sure you want to remove this country along with its provinces and cities?\')"><img src="'.get_option('home').'/wp-content/plugins/fs-real-estate-plugin/images/cart-x.png" border="0" alt="X"></a></td><td style="background: #EFEFEF;"><input type="text" name="c-'.$Countries->country_id.'" value="'.$Countries->country_name.'"></td></tr>';
		$Provinces = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."fsrep_provinces WHERE country_id = $Countries->country_id ORDER BY province_name");
		if (count($Provinces) > 0) {
			foreach ($Provinces as $Provinces) {
				echo '<tr><td style="background: #F5F5F5;"><a href="admin.php?page=fsrep_local&f=del&type=p&lid='.$Provinces->province_id.'" onClick="return confirm(\'Are you sure you want to remove this province along with its cities?\')"><img src="'.get_option('home').'/wp-content/plugins/fs-real-estate-plugin/images/cart-x.png" border="0" alt="X"></a></td><td style="background: #F5F5F5;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="p-'.$Provinces->province_id.'" value="'.$Provinces->province_name.'"></td></tr>';
				$Cities = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."fsrep_cities WHERE province_id = $Provinces->province_id ORDER BY city_name");
				if (count($Cities) > 0) {
					foreach ($Cities as $Cities) {
						echo '<tr><td><a href="admin.php?page=fsrep_local&f=del&type=cty&lid='.$Cities->city_id.'" onClick="return confirm(\'Are you sure you want to remove this city?\')"><img src="'.get_option('home').'/wp-content/plugins/fs-real-estate-plugin/images/cart-x.png" border="0" alt="X"></a></td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="cty-'.$Cities->city_id.'" value="'.$Cities->city_name.'"></td></tr>';
					}
				}
			}
		}
