...
function fsrep_filters() {
	...
	if (isset($_POST['submit'])) {
		if (isset($_POST['filter_name'])) {
			$wpdb->query("INSERT INTO ".$wpdb->prefix."fsrep_filters (filter_name, filter_map, filter_sorting) VALUES ('".$_POST['filter_name']."', '".$_POST['filter_map']."', '".$_POST['filter_sorting']."')");
	...
	if ($FSREPExtensions['ProFeatures'] != TRUE) {
		echo '</p><a href="http://www.firestormplugins.com/extensions/real-estate/" target="_blank" style="color: #749810;">Upgrade to the PRO Version to use multiple filters on a single page/post.</a></p>';
	}
	...
	echo '<h3>'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Create Custom Filter Label', 'Create Custom Filter').'</h3>';
	echo '<table class="widefat page fixed" cellspacing="0" border="1">
		...
		<tfoot>
		<tr>
		<th scope="col" class="manage-column" colspan="3"><input type="submit" name="submit" class="button-primary" value="'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Create Filter Label', 'Create Filter').'"></th>
		</tr>
		</tfoot>
		<tbody>';
		echo '<tr>';
		...
		echo '<td colspan="2"><select name="filter_map"><option value="1">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Yes Label', 'Yes').'</option><option value="0">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'No Label', 'No').'</option></select></td>';
		echo '</tr>';
		echo '<tr>';
		echo '<td>'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Show Sorting Options Label', 'Show Sorting Options').'</td>';
		echo '<td colspan="2"><select name="filter_sorting"><option value="1">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Yes Label', 'Yes').'</option><option value="0">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'No', 'No').'</option></select></td>';
		echo '</tr>';
		if (function_exists('fsrep_pro_field_default_sorting')) {
			echo fsrep_pro_field_default_sorting();
		} else {
