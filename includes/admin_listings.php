...
function fsrep_listings() {
	...
	$DenyListings = FALSE; if (function_exists('fsrep_pro_deny_listings')) { $DenyListings = fsrep_pro_deny_listings($user_ID); }
	$ListingLimit = FALSE; if (function_exists('fsrep_pro_listing_limit')) { $ListingLimit = fsrep_pro_listing_limit($user_ID); }
	...
	if (isset($_GET['f'])) {
		if ($_GET['f'] == 'add') {
			...
			if (isset($_POST['submit'])) {
				...
				if ($RegisterFormError == '') {
					if ($_POST['listing_id'] == '0') {
						...
						if (!isset($_POST['listing_address_city'])) { $_POST['listing_address_city'] = ''; }
						...
						$ListingVisibility = 1; if (function_exists('fsrep_pro_visibility') && $user_ID != 1) { $ListingVisibility = fsrep_pro_visibility(); }
						...
						if ($_POST['listing_address_number'] != '' && $_POST['listing_address_street'] != '' && $CityName != '' && $ProvinceName != '' && $_POST['listing_address_postal'] != '') {	
			...
			$PostNames = array('listing_label', 'listing_description', 'listing_sold', 'listing_price', 'listing_address_number', 'listing_address_street', 'listing_address_country', 'listing_address_province', 'listing_address_city', 'listing_address_postal', 'listing_contact_display', 'listing_contact_name', 'listing_contact_email', 'listing_contact_home_phone', 'listing_contact_cell_phone', 'listing_contact_special_instructions', 'listing_contact_form_email', 'listing_auto_coords', 'listing_zoom', 'listing_lat', 'listing_long', 'listing_video', 'listing_slideshow', 'listing_virtual_tour');
			foreach ($PostNames as $value) {
				if (!isset($_POST[$value])) { $_POST[$value] = ''; }
			}
			if ($_POST['listing_auto_coords'] == '') { $_POST['listing_auto_coords'] = 1; }
			...
			if (isset($_POST['submit'])) {
				...
				if ($RegisterFormError == '') {
					...
					$wpdb->query("UPDATE ".$wpdb->prefix."fsrep_listings SET listing_price = '".$_POST['listing_price']."' WHERE listing_id = ".$_POST['listing_id']);
					$wpdb->query("UPDATE ".$wpdb->prefix."fsrep_listings SET listing_price_num = '".str_replace(',','',$_POST['listing_price'])."' WHERE listing_id = ".$_POST['listing_id']);
					...
					$wpdb->query("UPDATE ".$wpdb->prefix."fsrep_listings SET listing_auto_coords = '".$_POST['listing_auto_coords']."' WHERE listing_id = ".$_POST['listing_id']);
					...
					if ($_POST['listing_auto_coords'] == 1) {
						$Coords = google_geocoder($_POST['listing_address_number'].' '.$_POST['listing_address_street'].' '.$CityName.' '.$ProvinceName.' '.$_POST['listing_address_postal'].' ', $FSREPconfig['GoogleMapAPI']);
						$wpdb->query("UPDATE ".$wpdb->prefix."fsrep_listings SET listing_long = '".$Coords[0]."', listing_lat = '".$Coords[1]."', listing_zoom = '16' WHERE listing_id = ".$_POST['listing_id']."; ");
					} else {
						$wpdb->query("UPDATE ".$wpdb->prefix."fsrep_listings SET listing_long = '".$_POST['listing_long']."', listing_lat = '".$_POST['listing_lat']."', listing_zoom = '".$_POST['listing_zoom']."' WHERE listing_id = ".$_POST['listing_id']."; ");
					}
			...
			$wpdb->query("DELETE FROM ".$wpdb->prefix."fsrep_listings_docs WHERE listing_id = ".$_GET['hid']);
			$wpdb->query("DELETE FROM ".$wpdb->prefix."fsrep_listings_pictures WHERE listing_id = ".$_GET['hid']);
			echo '<p></p>';
	...
	// DISPLAY LIST	
		...
		if ($ListingLimit == TRUE) {
			...
			echo '<h2>'.$HType.' House Listings <a href="'.get_option('home').'/wp-admin/admin.php?page=fsrep_listings&f=add" class="add-new-h2">Add New</a></h2>';
		}
	...
	echo '<table class="widefat page fixed" cellspacing="0" border="1">
		...
		foreach ($Listings as $Listings) {
			...
			if ($Listings->listing_sold == 1) { 
				$ListingStatus = 'Sold'; 
			}  elseif (isset($Listings->listing_featured)) { 
