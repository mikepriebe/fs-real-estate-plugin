...
function fsrep_house_listings() {
	...
	if (isset($_GET['f'])) {
		...
		echo '<p><a href="admin.php?page=fsrep_house_listings">Go Back to House Listings</a></p>';
	// DISPLAY LIST	
	} else {
		...
		<h2>Edit House Listings</h2>
		
		<form id="fsrep-house-listings" action="" method="get">
		
		<p><strong>Listings:</strong> <span class="count">(<?php echo $ListingCount; ?>)</span> | <strong>Sold:</strong> <span class="count">(<?php echo $SoldCount; ?>)</span></p>
		<p><br /></p>
		<p><strong><a href="admin.php?page=fsrep_house_listings&f=add">Add Listing</a></strong></p>
		<p><br /></p>
		<table width="100%" cellspacing="0" cellpadding="0" border="1">
		<tr style="border: 1px solid #999999;">
		<td width="50"><strong>ID</strong></td>
		<td><strong>Label / Address</strong></td>
		<td width="100"><strong>Status</strong></td>
		<td width="250"><strong>Listed</strong></td>
		<td width="200">&nbsp;</td>
		</tr>
		<?php
			...
			foreach ($Listings as $Listings) {
				...
				if ($Listings->listing_sold == 1) { $ListingStatus = 'Sold'; } 
				echo '<tr>';
				echo '<td>'.$Listings->listing_id.'</td>';
				echo '<td>'.fsrep_listing_name_gen($Listings->listing_id, $FSREPconfig['Listing Name Display']).'</td>';
				echo '<td>'.$ListingStatus.'</td>';
				echo '<td>'.$Listings->date_listed.'</td>';
				echo '<td><a href="admin.php?page=fsrep_house_listings&hid='.$Listings->listing_id.'&f=edit">edit</a> | <a href="admin.php?page=fsrep_house_listings&hid='.$Listings->listing_id.'&f=del">remove</a></td>';
				echo '</tr>';
			}
		?>
		</table>
