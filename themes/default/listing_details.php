<?php
	...
	if (file_exists($WPUploadDir['basedir'].'/fsrep/houses/large/'.$ListingDetails->listing_id.'.jpg')) { 
		$PageContent .= '<div id="fsrep-main-image" style="width: '.$FSREPMainImageMargin.'px;"><a id="fsrep-main-image-a" href="'.$WPUploadDir['baseurl'].'/fsrep/houses/large/'.$ListingDetails->listing_id.'.jpg" title="View Slideshow" class="thickbox" rel="fsreplisting"><img id="fsrep-main-image-img" src="'.$WPUploadDir['baseurl'].'/fsrep/houses/'.$ListingDetails->listing_id.'.jpg" alt="'.strip_tags(fsrep_listing_name_gen($ListingDetails->listing_id, $FSREPconfig['ListingNameDisplay'])).'" /></a></div>';
	}
	$PageContent .= '<div id="fsrep-aimages" style="margin-left: '.$FSREPImageMargin.'px;">';
	for ($i=1;$i<=50;$i++) {
		if (file_exists($WPUploadDir['basedir'].'/fsrep/houses/additional/small/'.$ListingDetails->listing_id.'-'.$i.'.jpg')) {
			$PageContent .= '<div class="fsrep-aimage"><a href="'.$WPUploadDir['baseurl'].'/fsrep/houses/additional/large/'.$ListingDetails->listing_id.'-'.$i.'.jpg" title="View Slideshow" class="thickbox" rel="fsreplisting" onmouseover="document.getElementById(\'fsrep-main-image-img\').src=\''.$WPUploadDir['baseurl'].'/fsrep/houses/additional/'.$ListingDetails->listing_id.'-'.$i.'.jpg\'; document.getElementById(\'fsrep-main-image-a\').rel=\'fsreplisting\'"><img src="'.$WPUploadDir['baseurl'].'/fsrep/houses/additional/small/'.$ListingDetails->listing_id.'-'.$i.'.jpg" class="full" /></a></div>';
		}
	}
	if (file_exists($WPUploadDir['basedir'].'/fsrep/houses/large/'.$ListingDetails->listing_id.'.jpg')) { 
		$PageContent .= '<div class="fsrep-aimage"><a href="'.$WPUploadDir['baseurl'].'/fsrep/houses/large/'.$ListingDetails->listing_id.'.jpg" title="View Slideshow" class="thickbox" rel="fsreplisting" onmouseover="document.getElementById(\'fsrep-main-image-img\').src=\''.$WPUploadDir['baseurl'].'/fsrep/houses/'.$ListingDetails->listing_id.'.jpg\'; document.getElementById(\'fsrep-main-image-a\').rel=\'fsreplisting\'"><img src="'.$WPUploadDir['baseurl'].'/fsrep/houses/small/'.$ListingDetails->listing_id.'.jpg" class="full" /></a></div>';
	}
	$PageContent .= '</div>';
	...
	*/
	...
	foreach ($Fields as $Fields) {
		if ($Fields->field_type == 'checkbox') {
			$HouseFieldInfo = $wpdb->get_results("SELECT listing_value FROM ".$wpdb->prefix."fsrep_listings_to_fields WHERE field_id = $Fields->field_id AND listing_id = $ListingDetails->listing_id");
			$FieldVisible = TRUE; if (function_exists('fsrep_pro_field_visibility')) { $FieldVisible = fsrep_pro_field_visibility($Fields->field_id); }
			if (count($HouseFieldInfo) > 0 && $FieldVisible == TRUE) {
				$PageContent .= '<p><strong>'.fsrep_text_translator('FireStorm Real Estate Plugin', $Fields->field_name.' Label', $Fields->field_name).': </strong>';
				foreach ($HouseFieldInfo as $HouseFieldInfo) {
					$PageContent .= $HouseFieldInfo->listing_value.', ';
				}
				$PageContent = substr($PageContent, 0, -2);
				$PageContent .= '</p>';
			}
		} else {
			...
			$FieldVisible = TRUE; if (function_exists('fsrep_pro_field_visibility')) { $FieldVisible = fsrep_pro_field_visibility($Fields->field_id); }
			if ($HouseFieldInfo != '' && $FieldVisible == TRUE) {
