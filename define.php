...
$FSREPCurrentPermission = $FSREPCurrentUser->roles[0];
...
if (isset($FSREPPostName)){
	$FSREPSearch = 'SELECT * FROM '.$wpdb->prefix.'fsrep_listings, '.$wpdb->prefix.'fsrep_listings_to_fields WHERE ';
	...
	$FSREPSearch .= ' '.$wpdb->prefix.'fsrep_listings_to_fields.listing_id = '.$wpdb->prefix.'fsrep_listings.listing_id AND '; // PRICE LOW
	...
	if ($FieldSearchSQL != '') {
		...
		$FSREPSearch .= ' '.$wpdb->prefix.'fsrep_listings_to_fields.listing_value IN('.$FieldSearchSQL.') '; // PRICE LOW
	}
	...
	$FSREPSearch .= ' GROUP BY '.$wpdb->prefix.'fsrep_listings.listing_id ORDER BY '.$wpdb->prefix.'fsrep_listings.listing_id DESC';
	if (function_exists('fsrep_custom_search_results')) { $FSREPSearch = fsrep_custom_search_results($FSREPSearch); }
	...
	if (isset($FSREPSearch['query'])) { $FSREPSearch = $FSREPSearch['query']; }
	$wpdb->insert($wpdb->prefix.'fsrep_search_queries',  array('query_value' => $FSREPSearch, 'glong' => $GMapSLong, 'glat' => $GMapSLat, 'gzoom' => $GMapSZoom));
