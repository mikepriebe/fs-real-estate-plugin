...
function fsrep_check_rules($post_id) {
	global $FSREPconfig,$_POST;
	if ($FSREPconfig['ListingsPageID'] == $post_id) {
		add_rewrite_rule($_POST['post_name'].'/(.+)','index.php?page_id='.$FSREPconfig['ListingsPageID'].'&LPage=$matches[1]','top');
		global $wp_rewrite;
		$wp_rewrite->flush_rules();
	}
}
function add_fsrep_rules() {   
	global $FSREPconfig;
	$ListingPage = get_page($FSREPconfig['ListingsPageID']);
	add_rewrite_rule($ListingPage->post_name.'/(.+)','index.php?page_id='.$FSREPconfig['ListingsPageID'].'&LPage=$matches[1]','top');
}
...
function fsrep_print_file_input($label, $name, $value, $length) {
	...
	return '<div id="fsrep_input"><div id="fsrep_input_label">'.$label.'</div><input type="file" name="'.$name.'" value="'.$value.'" size="'.$length.'"></div>';
}
function fsrep_print_password_input($label, $name, $value, $length) {
	$label = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' Label', $label);
	return '<div id="fsrep_input"><div id="fsrep_input_label">'.$label.'</div><input type="password" name="'.$name.'" value="'.$value.'" size="'.$length.'"></div>';
}
function fsrep_print_input($label, $name, $value, $length) {
	$label = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' Label', $label);
	return '<div id="fsrep_input"><div id="fsrep_input_label">'.$label.'</div><input type="text" name="'.$name.'" value="'.$value.'" size="'.$length.'"></div>';
}
function fsrep_print_textarea($label, $name, $value, $rows, $cols) {
	$label = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' Label', $label);
	return '<div id="fsrep_input"><div id="fsrep_input_label">'.$label.'</div><textarea name="'.$name.'" rows="'.$rows.'" cols="'.$cols.'">'.$value.'</textarea></div>';
}
function fsrep_print_selectbox($label, $name, $selvalue, $options, $onchange) {
	$label = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' Label', $label);
	$PageContent = '<div id="fsrep_input"><div id="fsrep_input_label">'.$label.'</div>';
	$PageContent .= '<select id="'.$name.'" name="'.$name.'"'; if ($onchange != '') { $PageContent .= 'onchange="'.$onchange.'"'; } $PageContent .= '>';
...
function fsrep_print_checkbox($label, $name, $selvalue, $options) {
	...
	foreach ($options as $key=>$value) {
		...
		$PageContent .= '<input type="checkbox" name="'.$name.'" value="'.$value.'" '.$selected.'>'.$key.' ';
	}
	$PageContent .= '</div>';
	return $PageContent;
}
function fsrep_print_radio($label, $name, $selvalue, $options) {
	...
	foreach ($options as $key=>$value) {
		$selected = '';
		if ($selvalue == $value) { $selected = 'checked'; }
		$PageContent .= '<input type="radio" name="'.$name.'" value="'.$value.'" '.$selected.'>'.$key.' ';
	}
	$PageContent .= '</div>';
	return $PageContent;
}
...
function fsrep_print_admin_textarea($label, $name, $value, $cols, $rows, $description) {
	$label = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' Label', $label);
	$description = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' About Label', $description);
...
function fsrep_print_admin_extension_input($label, $name, $filename, $buylink, $learnlink) {
	global $FSREPconfig,$FSREPExtensions,$FSREPExtensionDir;
	$label = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' Label', $label);
	$description = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' About Label', $description);
	echo '<tr><td>'.$label.'</td><td>';
	$FileExists = FALSE;
	if (file_exists($FSREPExtensionDir.$filename)) {
		echo '<input type="text" name="'.$name.'L'.'" value="'.$FSREPconfig[$name.'L'].'" style="width: 163px;"> <input type="submit" name="submit" class="button-secondary" value="'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Update Label', 'Update').'">';
		$FileExists = TRUE;
	} else {
		echo '<a href="'.$buylink.'" class="button-primary" target="_blank">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Buy Online Label', 'Buy Online').'</a> ';
		echo '<a href="'.$learnlink.'" class="button-secondary" target="_blank">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Learn More Label', 'Learn More').'</a> ';
	}
	echo '</td>';
	echo '<td style="font-weight: normal;">';
	if ($FileExists == TRUE) {
		echo 'Version '.$FSREPExtensions[$name.'V'];
		//echo '<a href="" class="button-primary" style="width: 64px; text-align: center;">'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Update Label', 'Update').'</a> ';
	} else {
	}
	echo '</td>';
	echo '</tr>';
}
function fsrep_print_admin_extension_upload($label, $name) {
	$label = fsrep_text_translator('FireStorm Real Estate Plugin', $label.' Label', $label);
	echo '<tr><td>'.$label.'</td><td><input type="file" name="'.$name.'" value="" style="width: 166px;"> <input type="submit" name="submit" class="button-secondary" value="Upload"></td></tr>';
}
...
function fsrep_listing_name_gen($ListingID, $ListingName) {
	...
	if ($ListingID != '') {
		...
		return stripslashes($ListingName);
	}
}
function fsrep_breadcrumbs() {
	...
	if ($ListingID != 0) {
		...
		$Breadcrumbs .= '> <a href="'.fsrep_listing_url_gen($ListingID).'">'.stripslashes($ListingLabel).'</a> ';
	}
	if ($Breadcrumbs != '') {
...
function fsrep_listing_url_gen($ListingID) {
	...
	if ($ListingID != '') {
		...
		$URL = get_option('home').'/'.$ListingHomeURL.'/'.$Listing->listing_id.'-'.fsrep_url_generator(substr($Listing->listing_label,0,30)).'/';
		$URL = str_replace(' ', '-', $URL);
		return $URL;
	}
}
	...
	if ($AllowAccess == TRUE) {
		if (isset($_POST['submit'])) {
			...
			if ($RegisterFormError == '') {
				...
				if (function_exists('fsrep_pro_category_add')) { fsrep_pro_category_add($ListingID); }
				...
				foreach ($Fields as $Fields) {
					if ($Fields->field_type != 'checkbox') {
						...
						$CheckBoxCount = count($CBValues);
						for($i=0;$i<=$CheckBoxCount;$i++) {
							if (isset($_POST[$i.'field'.$Fields->field_id]) && $CBValues[$i] != '') {
								$CBValue .= $_POST[$i.'field'.$Fields->field_id].', ';
								$wpdb->query("INSERT INTO ".$wpdb->prefix."fsrep_listings_to_fields (listing_id, field_id, listing_value) VALUES ($ListingID,$Fields->field_id,'".$CBValues[$i]."')");
							}
						}
					}
				}
...
function fsrep_listings_list($SQL, $CurrentURL) {
	...
	if (count($Listings) > 0) {
		...
		foreach ($Listings as $Listings) {
			...
			echo '<span style="color: #888888;">'.$Listings->listing_address_number.' '.$Listings->listing_address_street.' '.fsrep_get_address_name($Listings->listing_address_city, 'city').' '.fsrep_get_address_name($Listings->listing_address_province, 'province').' '.$Listings->listing_address_postal.'<br />';
			echo fsrep_text_translator('FireStorm Real Estate Plugin', 'Date Listed Label', 'Date Listed').': '.$Listings->date_listed.'</span><br />';
...
function fsrep_listings_display($category_id, $value, $type, $fpagination, $hpagination, $CategoryPE, $GoogleMap, $Filter) {
	global $post,$wpdb,$FSREPconfig,$GMapLat,$GMapLong,$GMapZoom,$CountryID,$ProvinceID,$CityID,$ListingHomeURL,$SearchQueryID,$FSREPThemeDir;
	...
	if ($type == 'search') {
	...
	} elseif ($type == 'member' || $type == 'company') {
		
		
		
		if (is_numeric($value)) { 
				$ListingsSQL = "SELECT * FROM ".$wpdb->prefix."fsrep_listings_to_users, ".$wpdb->prefix."fsrep_listings WHERE ".$wpdb->prefix."fsrep_listings_to_users.ID = $value AND ".$wpdb->prefix."fsrep_listings.listing_id = ".$wpdb->prefix."fsrep_listings_to_users.listing_id".$ModSQL." ORDER BY ".$_SESSION['Order'];
		}
		...
		if ($Filter != 0 && $type == 'filter') {
			if (function_exists('fsrep_pro_custom_filters')) {
				$ListingsSQL .= fsrep_pro_custom_filters($GoogleMap, $ListingSorting, $Filter, $ModSQL);
			}
		} elseif ($Filter != 0 && $type == 'category') {
			if (function_exists('fsrep_pro_custom_categories')) {
				$ListingsSQL .= fsrep_pro_custom_categories($GoogleMap, $ListingSorting, $Filter, $ModSQL);
			}
		} else {
	...
	$ListingsSQL = fsrep_sql_clean($ListingsSQL);
	if (isset($FSREPconfig['FSREPDebug']) && $FSREPconfig['FSREPDebug'] == 'Yes') { echo '<p>'.$ListingsSQL.'</p>'; }
	$Listings = $wpdb->get_results($ListingsSQL);
	$ListingCount = count($Listings);
	if (isset($FSREPconfig['FSREPDebug']) && $FSREPconfig['FSREPDebug'] == 'Yes') { echo '<p>'.$ListingCount.' Listings Found</p>'; }
	$Listings = array_slice($Listings, $PS, $PE);
	
	if ($GoogleMap == TRUE) {
		$PageContent .= '<div id="listings_map" style="width: 100%; height: 300px; border: 1px solid #999999; margin-bottom: 12px;"></div><br />';
	} else {
		$PageContent .= '';
	}
	...
	if ($FSREPconfig['DisablePageSorting'] == 0 && $ListingSorting == TRUE && function_exists('fsrep_pro_sorting')) {
		$PageContent .= fsrep_pro_sorting($ListingPEArray, $PE, $ListingCount, $TotalPages);
	}

	if (count($Listings) > 0) {
		if (function_exists('fsrep_pro_compare_checkbox') && $FSREPconfig['EnableCompare'] == 'Yes' && $ListingCompare == TRUE) { 
		...
		foreach ($Listings as $Listings) {
			...
			include($FSREPThemeDir.'listings_'.$FSREPconfig['ListingsOrientation'].'.php');
		}
...
function fsrep_search_form($FSREPSearchForm) {
	...
	$FSREPPriceRangeDisplay = TRUE;
	if (isset($FSREPconfig['HidePriceRange']) && $FSREPconfig['HidePriceRange'] == 'Yes') {
		$FSREPPriceRangeDisplay = FALSE;
	}
	if ($FSREPPriceRangeDisplay == TRUE) {
...
function fsrep_membership_settings_disabled() {
	...
	echo '<table class="widefat page fixed" cellspacing="0">
		...
		fsrep_print_admin_selectbox_disabled('Manage Listings Role', 'Requires Membership Extension');
