...
function google_geocoder($address, $api) {
	...
	if (isset($XML->Response->Placemark->Point->coordinates)) { $Coords = explode(',',$XML->Response->Placemark->Point->coordinates); }
...
function fsrep_listing_manager($CurrentURL) {
	...
	if (function_exists('fsrep_member_listing_limit')) {
		if (isset($MAPageID) && isset($post->ID) && $MAPageID == $post->ID && $FSREPCurrentPermission != $FSREPAdminPermissions) {
...
function fsrep_listings_display($category_id, $value, $type, $fpagination, $hpagination, $CategoryPE, $GoogleMap, $Filter) {
	...
	if ($type == 'search') {
		...
		$ListingsSQL = str_replace('  ',' ',$ListingsSQL);
		$ListingsSQL = str_replace('ORDER BY DESC','ORDER BY listing_id DESC',$ListingsSQL);
		$ListingsSQL = str_replace('DESC DESC','DESC',$ListingsSQL);
