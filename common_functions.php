...
function add_fsrep_rules() {   
	...
	add_rewrite_rule($page_data->post_name.'/(.+)','index.php?page_id='.$FSREPconfig['ListingsPageID'].'&LPage=$matches[1]','top');
}
...
function fsrep_print_admin_extension_input($label, $name, $filename, $buylink, $learnlink) {
	...
	if (file_exists($FSREPExtensionDir.$filename)) {
		echo '<input type="text" name="'.$name.'L'.'" value="'.$FSREPconfig[$name.'L'].'" style="width: 167px;"> <input type="submit" name="submit" class="button-secondary" value="'.fsrep_text_translator('FireStorm Real Estate Plugin', 'Update Label', 'Update').'">';
		$FileExists = TRUE;
	} else {
...
function fsrep_listings_display($category_id, $value, $type, $fpagination, $hpagination, $CategoryPE, $GoogleMap, $Filter) {
	...
	if ($type == 'search') {
		...
		if ($Filter != 0) {
			...
			$FieldSQL = "SELECT DISTINCT t1.listing_id FROM ".$wpdb->prefix."fsrep_listings_to_fields t1, ".$wpdb->prefix."fsrep_listings ";
			$Filters = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."fsrep_filters_details WHERE filter_id = ".$Filter);
			if (count($Filters) > 0) { $FieldSQL .= 'WHERE EXISTS '; }
			$FieldID = '';
			$ListingValue = '';
			$FieldValueSQL = '';
