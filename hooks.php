...
function fsrep_admin_pages() {
	...
	if ($FSREPCurrentPermission == $FSREPAdminPermissions) { $PageName = 'View All Listings'; } else { $PageName = 'View My Listings'; }
	...
	add_submenu_page(__FILE__, 'Filters &amp; Short Codes', 'Filters &amp; Short Codes', $FSREPAdminPermissions, 'fsrep_filters', 'fsrep_filters');
	add_submenu_page(__FILE__, 'Locations', 'Locations', $FSREPAdminPermissions, 'fsrep_local', 'fsrep_local');
	if (isset($FSREPExtensions['ProFeatures']) && $FSREPExtensions['ProFeatures'] == TRUE) {
		add_submenu_page(__FILE__, 'Categories', 'Categories', $FSREPAdminPermissions, 'fsrep_categories', 'fsrep_categories');
	}
...
function fsrep_admin_bar() {
	global $wp_admin_bar,$FSREPCurrentPermission,$FSREPAdminPermissions,$FSREPMemberPermissions,$FSREPExtensions;
	if ($FSREPCurrentPermission == $FSREPAdminPermissions || $FSREPCurrentPermission == $FSREPMemberPermissions) {
		if ($FSREPCurrentPermission == $FSREPAdminPermissions) { $ListingsLabel = 'View All Listings'; } else { $ListingsLabel = 'View My Listings'; }
		...
		'id' => 'fsrep_admin_all',
		'title' => fsrep_text_translator('FireStorm Real Estate Plugin', 'Admin Bar Label', $ListingsLabel),
	...
	if ($FSREPCurrentPermission == $FSREPAdminPermissions) {
		...
		if (isset($FSREPExtensions['ProFeatures']) && $FSREPExtensions['ProFeatures'] == TRUE) {
			$wp_admin_bar->add_menu( array(
			'parent' => 'fsrep_admin_bar',
			'id' => 'fsrep_admin_categories',
			'title' => fsrep_text_translator('FireStorm Real Estate Plugin', 'Admin Bar Label', 'Categories'),
			'href' => admin_url('admin.php?page=fsrep_categories'),
			));
		}
		$wp_admin_bar->add_menu( array(
		'parent' => 'fsrep_admin_bar',
		...
		'id' => 'fsrep_admin_support_phone',
		'title' => fsrep_text_translator('FireStorm Real Estate Plugin', 'Admin Bar Label', 'Phone Support'),
		'href' => 'http://www.firestormplugins.com/support/',
		'meta' => array('target' => '_blank',),
		));
		$wp_admin_bar->add_menu( array(
		'parent' => 'fsrep_admin_support',
		'id' => 'fsrep_admin_support_ticket',
		'title' => fsrep_text_translator('FireStorm Real Estate Plugin', 'Admin Bar Label', 'Create Support Ticket'),
		'href' => 'http://www.firestormplugins.com/tickets/',
		'meta' => array('target' => '_blank',),
		));
		$wp_admin_bar->add_menu( array(
		'parent' => 'fsrep_admin_support',
		...
		if ($FSREPExtensions['ProFeatures'] != TRUE) {
			$wp_admin_bar->add_menu( array(
			'parent' => 'fsrep_admin_bar',
			'id' => 'fsrep_admin_upgrade',
			'title' => fsrep_text_translator('FireStorm Real Estate Plugin', 'Admin Bar Label', 'Upgrade to PRO Version'),
			'href' => 'http://www.firestormplugins.com/extensions/real-estate/',
			'meta' => array('class' => 'fsrep-green-link','target' => '_blank',),
			));
		}
	}
}
...
function fsrep_head() {
	...
	if ($FSREPconfig['GoogleMap'] == 1) {
		if (isset($post)) {
			if ($ListingID != 0 || $CityID != 0 || $ProvinceID != 0 || $CountryID != 0 || preg_match("/fsrep-listing-/i", $post->post_content) || preg_match("/fsrep-filter-/i", $post->post_content) || preg_match("/fsrep-filter /i", $post->post_content) || preg_match("/fsrep-map /i", $post->post_content) || $post->ID == $FSREPconfig['ListingsPageID']) {
				$ShowGoogleMap = TRUE;
	...
	// META TAGS
	$METADescription = '';
	if (isset($FSREPconfig['ListingsPageID']) && isset($post)) {
		if ($post->ID == $FSREPconfig['ListingsPageID']) {
			$ShowGoogleMap = TRUE;
			$METADescription = '';
			if ($ListingID != 0) {
	...
	// SHOW GOOGLE MAP
	if (preg_match("/fsrep-listing-/i", $post->post_content)) {
		$ListingID = explode('[fsrep-listing-', $post->post_content);
		$ListingID = explode(']', $ListingID[1]);
	...
	if (isset($post)) {
		if ($post->ID == $FSREPconfig['ListingsPageID']) {
			if (preg_match("/fsrep-filter-/i", $post->post_content)) {
				...
				$FilterID = $FilterID[0];
				$ShowGoogleMap = TRUE;
			} elseif (preg_match("/fsrep-category-/i", $post->post_content)) {
				$CatID = explode('[fsrep-category-', $post->post_content);
    ...
    if ($FilterID != 0) {
			...
			$MarkerListingURL = 'category='.$CategoryID;
			$GMapLat = $FSREPconfig['MapCenterLat'];
			$GMapLong = $FSREPconfig['MapCenterLong'];
			$GMapZoom = $FSREPconfig['MapCenterZoom'];
		} elseif ($FSREPSearch != '') {
		...
		if (isset($FSREPconfig['ListingMapTypeId']) && $ListingID != 0) { $GMapType = $FSREPconfig['ListingMapTypeId']; }
      ...
      function FSREPMap() {
       ...
       var myOptions = {
          ...
          zoom: <?php echo $GMapZoom; ?>,
					<?php if (function_exists('fsrep_pro_map_hook')) { fsrep_pro_map_hook(); } ?>
