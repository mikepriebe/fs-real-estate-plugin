...
function fsrep_head() {
	...
	if ($FSREPconfig['GoogleMap'] == 1) {
		if (isset($post)) {
			if ($ListingID != 0 || $CityID != 0 || $ProvinceID != 0 || $CountryID != 0 || preg_match("/fsrep-listing-/i", $post->post_content) || preg_match("/fsrep-filter-/i", $post->post_content) || preg_match("/fsrep-filter /i", $post->post_content) || preg_match("/fsrep-map /i", $post->post_content) || $post->ID == $LPageID) {
				$ShowGoogleMap = TRUE;
