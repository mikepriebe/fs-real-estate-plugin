...
function fsrep_admin_pages() {
	...
	add_menu_page('Real Estate', 'Real Estate', $FSREPListingsPerm, __FILE__, 'fsrep_admin_home');
	...
	add_submenu_page(__FILE__, 'Custom Fields', 'Custom Fields', $FSREPListingsPerm, 'fsrep_fields', 'fsrep_fields');
	add_submenu_page(__FILE__, 'Filters', 'Filters', $FSREPListingsPerm, 'fsrep_filters', 'fsrep_filters');
	add_submenu_page(__FILE__, 'Locations', 'Locations', $FSREPListingsPerm, 'fsrep_local', 'fsrep_local');
	if ($FSREPMembers == TRUE) {
		if ($FSREPconfig['EnableListingPlans'] == 'Yes') {
			add_submenu_page(__FILE__, 'Plan Pricing', 'Plan Pricing', $FSREPListingsPerm, 'fsrep_plans', 'fsrep_plans');
		}
	}
	add_submenu_page(__FILE__, 'Settings', 'Settings', $FSREPListingsPerm, 'fsrep_settings', 'fsrep_settings');
}
       ...
       var myOptions = {
        ...
        var map = new google.maps.Map(document.getElementById("listings_map"),
            myOptions);
				...
				xmlhttp.open("GET","<?php echo get_option('home'); ?>/wp-content/plugins/fs-real-estate-plugin/xml/marker_listings.xml?<?php echo $MarkerListingURL; ?>",false);
				xmlhttp.send();
