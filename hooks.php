...
function fsrep_admin_pages() {
	global $user_ID,$FSREPconfig,$FSREPMembers,$FSREPAdminPermissions,$FSREPMemberPermissions,$FSREPExtensions;
	...
	if ($FSREPExtensions['Membership'] == TRUE) {
		if (isset($FSREPconfig['EnableListingPlans'])) {
			if ($FSREPconfig['EnableListingPlans'] == 'Yes') {
				add_submenu_page(__FILE__, 'My Profile', 'My Profile', $FSREPMemberPermissions, __FILE__, 'fsrep_profile');
			}
		}
	}
	add_submenu_page(__FILE__, $PageName, $PageName, $FSREPMemberPermissions, 'fsrep_listings', 'fsrep_listings');
