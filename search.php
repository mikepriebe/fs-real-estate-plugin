...
if(isset($_GET['CityID'])){
	if($_GET['CityID'] != '' && is_numeric($_GET['CityID'])){
		...
		if (substr($HTTPREFERER[0], -13) == '/add-listing/') {
			echo "obj.options[obj.options.length] = new Option('Select ".$FSREPconfig['RegionLabel'].",'');\n";
		} else {
			echo "obj.options[obj.options.length] = new Option('Show All of ".$FSREPCity->city_name."','');\n";
		}
		$count = 1;
