...
function fsrep_content($content) {
	global $LPageID,$LOPageID,$MAPageID,$MembersPageID,$post,$wpdb,$wp_rewrite,$user_ID,$current_user,$FSREPconfig,$CityID,$ProvinceID,$CountryID,$RequestURI,$FSREPMembers,$FSREPExtensions,$FSREPThemeDir;
	$PageContent = '';
	if ($post->ID == $FSREPconfig['ListingsPageID']) {
		...
		if ($ViewListings == 'yes') {
			if (is_numeric(substr($RequestURI[1], 0, 1))) {
				...
				if (function_exists('fsrep_pro_extra_text')) {
					$PageContent .= fsrep_pro_extra_text('FSREPListingDetailsText');
				}
				include($FSREPThemeDir.'listing_details.php');
				if (function_exists('fsrep_pro_extra_text')) {
					$PageContent .= fsrep_pro_extra_text('FSREPListingDetailsBText');
				}
			...
			} elseif (isset($FSREPconfig['MembershipURL']) && substr($RequestURI[1], 0, strlen($FSREPconfig['MembershipURL'])) == $FSREPconfig['MembershipURL']) {
				if (function_exists('fsrep_member_bio_page')) {
					$PageContent .= fsrep_member_bio_page();
				}
			} elseif (substr($RequestURI[1],0,4) == 'cat-') {
				if (function_exists('fsrep_category_listings')) {
					$PageContent .= fsrep_category_listings(str_replace('cat-','',$RequestURI[1]));
				}
			} else {
				$Type = '';
				$Value = '';
				...
				if (function_exists('fsrep_pro_extra_text')) {
					$PageContent .= fsrep_pro_extra_text('FSREPListingsText');
				}				
				$PageContent .= fsrep_listings_display(0, $Value, $Type, 0, 0, 0, $FSREPconfig['GoogleMap'], 0);
				if (function_exists('fsrep_pro_extra_text')) {
					$PageContent .= fsrep_pro_extra_text('FSREPListingsBText');
				}				
			}
		} else {
		...
		if (preg_match("/fsrep-filter-/i", $content)) {
			if (function_exists('fsrep_pro_custom_filter_replace') && function_exists('fsrep_pro_custom_filters')) {
				$content = fsrep_pro_custom_filter_replace($content,$ViewListings);
			} else {
				$content = 'Please upgrade to the PRO version to use custom filters.';
			}
		} elseif (preg_match("/fsrep-categories/i", $content)) {
			if (function_exists('fsrep_pro_custom_categories_display')) {
				$content = fsrep_pro_custom_categories_display($content,$ViewListings);
			} else {
				$content = 'Please upgrade to the PRO version to use categories.';
			}
		} elseif (preg_match("/fsrep-category-/i", $content)) {
			if (function_exists('fsrep_pro_custom_category_replace')) {
				$content = fsrep_pro_custom_category_replace($content,$ViewListings);
			} else {
				$content = 'Please upgrade to the PRO version to use categories.';
			}
